...
use khaledBundle\Entity\ProductM;
...
class DefaultController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $products = $em->getRepository('khaledBundle:Product')->findBy(['state' => 2]);
        $query = $em->createQuery(
        'SELECT p
        FROM khaledBundle:ProductM p
        ORDER BY p.id DESC'
        );
    ...
    public function add_cartAction(Request $request)
    {
        ...
        //if ($request->isXmlHttpRequest() || $request->query->get('showJson') == 1) {
            ...
            $em = $this->getDoctrine()->getManager();
            $c = $em->getRepository(Cart::class)->findBy(['idUser' => $user->getid(),"idProduct"=> $id ]);
            if(count($c) == 0) {
                $cart = new Cart();
        ...
        */
            ...
            if ($request->isXmlHttpRequest() || $request->query->get('showJson') == 1) {
                ...
                if ($action=="produit" AND $signal=="not done")
                {
                    ...
                    foreach ($orders as $c)
                    {
                        $html.="<input type=\"checkbox\" data-categorie='' name=\"sport\" value=".$c["id"].">".$c["title"]."<br>";
                    }
